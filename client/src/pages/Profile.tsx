...
interface DayStatus {
  ...
  label: string;
  shortLabel: string;
  status: "worked_out" | "missed" | "inactive";
  workout?: SafeWorkout;
}
...
function Profile() {
  ...
  const last14Days: DayStatus[] = useMemo(() => {
    ...
    for (let offset = 13; offset >= 0; offset -= 1) {
      ...
      const shouldCountAsMissed =
        firstWorkoutDate !== null && date >= firstWorkoutDate;
    ...
    return days;
  }, [firstWorkoutDate, workoutMap]);

  const missedDays = firstWorkoutDate
    ? last14Days.filter((day) => day.status === "missed")
    : [];
  const completedDays = workouts.length > 0
    ? last14Days.filter((day) => day.workout)
    : [];
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.16),_transparent_22%),linear-gradient(135deg,_#020617_0%,_#0f172a_50%,_#020617_100%)] px-4 py-8 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        >
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/70">
                {user?.profileImage ? (
                  <img
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-cyan-400/15 text-3xl font-semibold text-cyan-100">
              ...
              <div className="space-y-3">
                ...
                <h1 className="text-3xl font-semibold leading-tight sm:text-4xl">
                  {user?.name || "Your"} training history
                </h1>
                <p className="max-w-2xl text-sm text-slate-300 sm:text-base">
                  {user?.email}
                </p>
                <div className="flex flex-wrap items-center gap-3">
                  <label className="inline-flex cursor-pointer items-center rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2 text-sm font-medium text-cyan-100 transition hover:bg-cyan-400/15">
                    <input
                      type="file"
              ...
              >
                ...
                {day.workout ? (
                  ...
                  <p className="mt-4 text-sm text-slate-400">
                    {day.status === "missed" ? "No workout logged" : "No history yet"}
                  </p>
                )}
              </div>
            ))}
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          >
            <div className="mb-5">
              <h2 className="text-2xl font-semibold">Missed days</h2>
